...
from robot_ros import KUKASAKE
...
class ENVIRONMENT:
    ...
    def step(self, action):
        ...
        robot_sim_states = np.array([i[0] for i in self.robot.bc.getJointStates(self.robot.robot_id, self.robot.arm_joint_ids)])
        while np.linalg.norm(self.robot.arm_joint_states - robot_sim_states) > 1e-2:
            for position, joint_id in zip(self.robot.arm_joint_states, self.robot.arm_joint_ids):
                self.robot.bc.setJointMotorControl2(self.robot.robot_id, joint_id, self.robot.bc.POSITION_CONTROL,
                                                    targetPosition=position, force=self.robot.arm_force)
            self.bc.stepSimulation()
